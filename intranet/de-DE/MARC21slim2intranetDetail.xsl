...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="marc:record">
        ...
        <xsl:variable name="SubjectModifier">
            <xsl:if test="marc:sysprefs/marc:syspref[@name='TraceCompleteSubfields']='1'">,complete-subfield</xsl:if>
        </xsl:variable>
        ...
        <xsl:variable name="materialTypeLabel">
            <xsl:choose>
                <xsl:when test="$leader19='a'">Gesamtaufnahme</xsl:when>
                <xsl:when test="$leader6='a'">
                    <xsl:choose>
                        <xsl:when test="$leader7='c' or $leader7='d' or $leader7='m'">Text</xsl:when>
                        <xsl:when test="$leader7='i' or $leader7='s'">
                            <xsl:choose>
                                <xsl:when test="substring($controlField008,22,1)!='m'">Fortlaufende Ressource</xsl:when>
                                <xsl:otherwise>Reihen</xsl:otherwise>
                            </xsl:choose>
                        </xsl:when>
                        <xsl:when test="$leader7='a' or $leader7='b'">Aufsatz</xsl:when>
                    </xsl:choose>
                </xsl:when>
                <xsl:when test="$leader6='t'">Text</xsl:when>
                <xsl:when test="$leader6='o'">Medienkombination</xsl:when>
                <xsl:when test="$leader6='p'">Medienkombination</xsl:when>
                <xsl:when test="$leader6='m'">Computerdatei</xsl:when>
                <xsl:when test="$leader6='e' or $leader6='f'">Karte</xsl:when>
                <xsl:when test="$leader6='g'">Film</xsl:when>
                <xsl:when test="$leader6='k'">Bild</xsl:when>
                <xsl:when test="$leader6='r'">Objekt</xsl:when>
                <xsl:when test="$leader6='j'">Musik</xsl:when>
                <xsl:when test="$leader6='i'">Ton</xsl:when>
                <xsl:when test="$leader6='c' or $leader6='d'">Noten</xsl:when>
            </xsl:choose>
        </xsl:variable>
        ...
        <xsl:call-template name="showAuthor">
            <xsl:with-param name="authorfield"
                            select="marc:datafield[(@tag=700 or @tag=710 or @tag=711) and not(@ind2=2) and not(marc:subfield[@code='i'])]"/>
            <xsl:with-param name="UseAuthoritiesForTracings" select="$UseAuthoritiesForTracings"/>
            <xsl:with-param name="AuthorLinkSortOrder" select="$AuthorLinkSortOrder"/>
            <xsl:with-param name="AuthorLinkSortBy" select="$AuthorLinkSortBy"/>
        </xsl:call-template>
        ...
        <xsl:if test="$DisplayIconsXSLT!='0' and $materialTypeCode!=''">
            <span class="results_summary type">
                ...
                <xsl:element name="img">
                    <xsl:attribute name="class">materialtype mt_icon_<xsl:value-of select="$materialTypeCode"/>
                    </xsl:attribute>
                    <xsl:attribute name="src">/intranet-tmpl/prog/img/famfamfam/<xsl:value-of
                            select="$materialTypeCode"/>.png
                    </xsl:attribute>
        ...
        <xsl:if test="$display880">
            <xsl:call-template name="m880Select">
                ...
                <xsl:with-param name="label">Reihen:</xsl:with-param>
        ...
        <xsl:if test="$show_analytics_link='1'">
            <xsl:element name="span">
                <xsl:attribute name="class">results_summary analytics
                    <xsl:value-of select="$leader7_class"/>
                </xsl:attribute>
                <span class="label">Aufsätze:</span>
                <a>
                    <xsl:choose>
                        <xsl:when test="$UseControlNumber = '1' and marc:controlfield[@tag=001]">
                            <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=rcn:<xsl:value-of
                                    select="str:encode-uri(marc:controlfield[@tag=001], true())"/>+AND+(bib-level:a+OR+bib-level:b)
                            </xsl:attribute>
                        </xsl:when>
                        <xsl:otherwise>
                            ...
                            <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=<xsl:value-of
                                    select="str:encode-uri($title_query, true())"/>
                            </xsl:attribute>
                        </xsl:otherwise>
                    </xsl:choose>
                    <xsl:text>Zeige unselbstständige Werke</xsl:text>
                </a>
            </xsl:element>
        </xsl:if>
        ...
        <xsl:variable name="title"
                      select="str:encode-uri(translate(marc:datafield[@tag=245]/marc:subfield[@code='a'], '/', ''), true())"/>
        <xsl:if test="$show_volumes_link='1' and ($materialTypeCode='ST' or substring($controlField008,22,1)='m')">
            <span class="results_summary volumes">
                <span class="label">Bände:</span>
                <a>
                    <xsl:choose>
                        <xsl:when test="$UseControlNumber = '1' and marc:controlfield[@tag=001]">
                            <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=rcn:<xsl:value-of
                                    select="str:encode-uri(marc:controlfield[@tag=001], true())"/>+NOT+(bib-level:a+OR+bib-level:b)
                            </xsl:attribute>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:attribute name="href">
                                /cgi-bin/koha/catalogue/search.pl?q=title-series,phr:"<xsl:value-of select="$title"/>"+OR+host-item,phr="<xsl:value-of
                                    select="$title"/>"+NOT+(bib-level:a+OR+bib-level:b)
                            </xsl:attribute>
                        </xsl:otherwise>
                    </xsl:choose>
                    <xsl:text>Zeige Bände</xsl:text>
                </a>
            </span>
        </xsl:if>
        ...
        <xsl:if test="$leader19='c'">
            <span class="results_summary set">
                <span class="label">Gesamtaufnahme:</span>
                <xsl:for-each select="marc:datafield[@tag=773]">
                    <a>
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=ti,phr:<xsl:value-of
                                        select="str:encode-uri(translate(//marc:datafield[@tag=245]/marc:subfield[@code='a'], '.', ''), true())"/>
                                </xsl:attribute>
                            </xsl:otherwise>
                        </xsl:choose>
                ...
                <span class="results_summary publisher">
                    ...
                    <xsl:for-each select="marc:datafield[@tag=260]">
                        <xsl:for-each select="marc:subfield">
                            <xsl:if test="@code='a'">
                                <a>
                                    <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=pl:"<xsl:value-of
                                            select="str:encode-uri(current(), true())"/>"
                                    </xsl:attribute>
                            ...
                            <xsl:if test="@code='b'">
                                <a>
                                    <xsl:attribute name="href">
                                        /cgi-bin/koha/catalogue/search.pl?q=Provider:"<xsl:value-of
                                            select="str:encode-uri(current(), true())"/>"
                                    </xsl:attribute>
                                    <xsl:value-of select="current()"/>
                                </a>
                            ...
                            <xsl:if test="@code='c'">
                                <a>
                                    <xsl:attribute name="href">
                                        /cgi-bin/koha/catalogue/search.pl?q=copydate:"<xsl:value-of
                                            select="str:encode-uri(current(), true())"/>"
                                    </xsl:attribute>
                                    <xsl:value-of select="current()"/>
                                </a>
        ...
        <xsl:if test="$display880">
            <xsl:call-template name="m880Select">
                ...
                <xsl:with-param name="label">Auflage:</xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        ...
        <xsl:if test="$display880">
            <xsl:call-template name="m880Select">
                ...
                <xsl:with-param name="label">Kartografisches Daten:</xsl:with-param>
            </xsl:call-template>
        </xsl:if>

        <xsl:if test="marc:datafield[@tag=255]">
            <span class="results_summary cartographic_data">
                <span class="label">Kartografisches Daten:</span>
        ...
        <xsl:if test="marc:datafield[@tag=336] or marc:datafield[@tag=337] or marc:datafield[@tag=338]">
            <span class="results_summary" id="content_type">
                ...
                <xsl:if test="marc:datafield[@tag=338]">
                    <span class="label">Datenträgertyp:</span>
        ...
        <xsl:if test="marc:datafield[@tag=013]">
            <span class="results_summary patent_info">
                <span class="label">Patentinformation:</span>
        ...
        <xsl:if test="marc:datafield[@tag=088]">
            <span class="results_summary report_number">
                <span class="label">Reportnummer:</span>
            ...
            <span class="results_summary other_title">
                ...
                <xsl:for-each select="marc:datafield[@tag=246]">
                    ...
                    <xsl:if test="@ind1=1 and not(marc:subfield[@code='i'])">
                        <xsl:choose>
                            <xsl:when test="@ind2=0">[Teil des Titels]</xsl:when>
                            <xsl:when test="@ind2=1">[Paralleltitel]</xsl:when>
                            <xsl:when test="@ind2=2">[Schlüsseltitel]</xsl:when>
                            <xsl:when test="@ind2=3">[Weiterer Titel]</xsl:when>
                            <xsl:when test="@ind2=4">[Umschlagtitel]</xsl:when>
                            <xsl:when test="@ind2=5">[Titel von zusätzlicher Titelseite]</xsl:when>
                            <xsl:when test="@ind2=6">[Beschriftung]</xsl:when>
                            <xsl:when test="@ind2=7">[Kolumnentitel]</xsl:when>
                            <xsl:when test="@ind2=8">[Rückentitel]</xsl:when>
                        </xsl:choose>
                    </xsl:if>
        ...
        <xsl:if test="$display880">
            <xsl:call-template name="m880Select">
                ...
                <xsl:with-param name="label">Einheitssachtitel:</xsl:with-param>
            </xsl:call-template>
        </xsl:if>

        <xsl:if test="marc:datafield[@tag=130]|marc:datafield[@tag=240]|marc:datafield[@tag=730][@ind2!=2]">
            <span class="results_summary uniform_title">
                <span class="label">Einheitssachtitel:</span>
                <xsl:for-each
        ...
        <xsl:if test="marc:datafield[@tag=700][marc:subfield[@code='i']] or marc:datafield[@tag=710][marc:subfield[@code='i']] or marc:datafield[@tag=711][marc:subfield[@code='i']]">
            <span class="results_summary related_works">
                <span class="label">Verwandte Werke:</span>
                <xsl:for-each
                        select="marc:datafield[@tag=700][marc:subfield[@code='i']] | marc:datafield[@tag=710][marc:subfield[@code='i']] | marc:datafield[@tag=711][marc:subfield[@code='i']]">
        ...
        <xsl:if test="marc:datafield[@tag=700][@ind2=2 and not(marc:subfield[@code='i'])] or marc:datafield[@tag=710][@ind2=2 and not(marc:subfield[@code='i'])] or marc:datafield[@tag=711][@ind2=2 and not(marc:subfield[@code='i'])]">
            <span class="results_summary contained_works">
                <span class="label">Enthaltene Werke:</span>
                <xsl:for-each
                        select="marc:datafield[@tag=700][@ind2=2][not(marc:subfield[@code='i'])] | marc:datafield[@tag=710][@ind2=2][not(marc:subfield[@code='i'])] | marc:datafield[@tag=711][@ind2=2][not(marc:subfield[@code='i'])]">
                    <xsl:variable name="str">
                        <xsl:call-template name="subfieldSelect">
                            <xsl:with-param name="codes">abcdfghiklmnporstux</xsl:with-param>
                        </xsl:call-template>
        ...
        <xsl:if test="marc:datafield[substring(@tag, 1, 1) = '6' and not(@tag=655)]">
            <span class="results_summary subjects">
                <span class="label">Schlagwörter:</span>
                    ...
                    <a>
                        <xsl:choose>
                            ...
                            <xsl:when test="marc:subfield[@code=9] and $UseAuthoritiesForTracings='1'">
                                <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=an:<xsl:value-of
                            ...
                            <xsl:when test="$TraceSubjectSubdivisions='1'">
                                <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=
                                    <xsl:call-template name="subfieldSelectSubject">
                                        ...
                                        <xsl:with-param name="prefix">(su<xsl:value-of select="$SubjectModifier"/>:<xsl:value-of
                                                select="$TracingQuotesLeft"/>
                                        </xsl:with-param>
                                        <xsl:with-param name="suffix"><xsl:value-of select="$TracingQuotesRight"/>)
                                        </xsl:with-param>
                            ...
                            <!-- #1807 Strip unwanted parenthesis from subjects for searching -->
                            <xsl:otherwise>
                                <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=su<xsl:value-of
                                        select="$SubjectModifier"/>:<xsl:value-of select="$TracingQuotesLeft"/><xsl:value-of
                                        select="str:encode-uri(translate(marc:subfield[@code='a'],'()',''), true())"/><xsl:value-of
                                        select="$TracingQuotesRight"/>
                                </xsl:attribute>
                            </xsl:otherwise>
                        </xsl:choose>
                ...
                <xsl:for-each select="marc:datafield[@tag=655]">
                    <a>
                        <xsl:choose>
                            ...
                            <xsl:when test="$TraceSubjectSubdivisions='1'">
                                <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=
                                    <xsl:call-template name="subfieldSelect">
                                        ...
                                        <xsl:with-param name="delimeter">AND</xsl:with-param>
                                        <xsl:with-param name="prefix">(index-term-genre<xsl:value-of
                                                select="$SubjectModifier"/>:<xsl:value-of select="$TracingQuotesLeft"/>
                                        </xsl:with-param>
                                        <xsl:with-param name="suffix"><xsl:value-of select="$TracingQuotesRight"/>)
                                        </xsl:with-param>
                                        <xsl:with-param name="urlencode">1</xsl:with-param>
                            ...
                            <xsl:otherwise>
                                <xsl:attribute name="href">
                                    /cgi-bin/koha/catalogue/search.pl?q=index-term-genre<xsl:value-of
                                        select="$SubjectModifier"/>:<xsl:value-of select="$TracingQuotesLeft"/><xsl:value-of
                                        select="marc:subfield[@code='a']"/><xsl:value-of select="$TracingQuotesRight"/>
                                </xsl:attribute>
                            </xsl:otherwise>
                        </xsl:choose>
                        <xsl:call-template name="subfieldSelect">
        ...
        <xsl:if test="marc:datafield[@tag=776]">
            <span class="results_summary add_physical_form">
                <span class="label">Andere physische Formen:</span>
            ...
            <xsl:for-each select="marc:datafield[@tag=777]">
                <xsl:if test="@ind1 != 1">
                    <span class="results_summary issued_with">
                        <span class="label">
                            <xsl:choose>
                                ...
                                <xsl:otherwise>
                                    <xsl:text>Erschienen mit:</xsl:text>
                                </xsl:otherwise>
                            </xsl:choose>
                            <xsl:text> </xsl:text>
                        </span>
                    ...
                    <xsl:if test="marc:subfield[@code='n']">
                        <xsl:text> </xsl:text>
                        <span class="results_summary in_note">
        ...
        <xsl:if test="marc:datafield[@tag=082]">
            <span class="results_summary ddc">
                <span class="label">DDC-Klassifikation:</span>
        ...
        <xsl:if test="marc:datafield[@tag=050]">
            <span class="results_summary loc">
                <span class="label">LOC-Klassifikation:</span>
        ...
        <xsl:if test="marc:datafield[@tag=060]">
            <span class="results_summary nlm">
                <span class="label">NLM-Klassifikation:</span>
        ...
        <xsl:if test="marc:datafield[@tag=084]">
            <span class="results_summary oc">
                <span class="label">Andere Klassifikation:</span>
            ...
            <div class="results_summary contents">
                <xsl:choose>
                    ...
                    <xsl:when test="marc:datafield[@tag=505]/@ind1=1">
                        <span class="label">Unvollständige Inhalte:</span>
                    </xsl:when>
                    <xsl:when test="marc:datafield[@tag=505]/@ind1=2">
                        <span class="label">Teilweise enthalten:</span>
                    </xsl:when>
                </xsl:choose>
        ...
        <xsl:if test="marc:datafield[@tag=508]">
            <span class="results_summary prod_credits">
                <span class="label">Herstellerabspann:</span>
        ...
        <xsl:if test="marc:datafield[@tag=583]">
            <span class="results_summary actionnote">
                <span class="label">Bearbeitungsvermerk:</span>
        ...
        <xsl:if test="marc:datafield[@tag=530]">
            <span class="results_summary additionalforms">
                <span class="label">Weitere erhältliche physische Ausgaben:</span>
                ...
                <span class="label">
                    <xsl:choose>
                        ...
                        <xsl:when test="@ind1=2">
                            <xsl:text>Bereich und Inhalt: </xsl:text>
                        </xsl:when>
                        <xsl:when test="@ind1=3">
                            <xsl:text>Zusammenfassung: </xsl:text>
                        </xsl:when>
                        <xsl:when test="@ind1=4">
                            <xsl:text>Inhaltsempfehlung: </xsl:text>
                        </xsl:when>
        ...
        <xsl:if test="marc:datafield[@tag=866]">
            <span class="results_summary holdings_note basic_bibliographic_unit">
        ...
        <xsl:if test="marc:datafield[@tag=867]">
            <span class="results_summary holdings_note supplementary_material">
                <span class="label">Supplemente:</span>
        ...
        <xsl:if test="marc:datafield[@tag=868]">
            <span class="results_summary holdings_note indexes">
            ...
            <span class="results_summary original_language_entry">
                <xsl:choose>
                    <xsl:when test="@ind2!=8 or not(marc:subfield[@code='i'])">
                        ...
                        <span class="label">Übersetzung von:</span>
                    </xsl:when>
                    <xsl:otherwise>
                        <span class="label">
                            <xsl:value-of select="marc:subfield[@code='i']"/>
                ...
                <xsl:for-each select="marc:datafield[@tag=770]">
                    ...
                    <a>
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=ti,phr:<xsl:value-of
                                        select="str:encode-uri(translate(marc:subfield[@code='t'], '()', ''),true())"/>
                                </xsl:attribute>
                            </xsl:otherwise>
                        </xsl:choose>
                        <xsl:choose>
        ...
        <xsl:if test="marc:datafield[@tag=772]">
            <span class="results_summary supplement_parent">
                <span class="label">Beilage zu:</span>
        ...
        <xsl:if test="marc:datafield[@tag=775]">
            <span class="results_summary other_editions">
                <span class="label">Andere Ausgaben:</span>
                <xsl:for-each select="marc:datafield[@tag=775]">
                    ...
                    <a>
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=<xsl:value-of
                                        select="str:encode-uri($relation_query, true())"/>
                                </xsl:attribute>
                            </xsl:otherwise>
                        </xsl:choose>
                        <xsl:call-template name="subfieldSelect">
            ...
            <xsl:for-each select="marc:datafield[@tag=780]">
                <xsl:if test="@ind1=0">
                    <span class="results_summary preceeding_entry">
                        <xsl:choose>
                            <xsl:when test="@ind2=0">
                                <span class="label">Fortsetzung von:</span>
                            </xsl:when>
                            <xsl:when test="@ind2=1">
                                <span class="label">Vorgänger teilweise:</span>
                            </xsl:when>
                            <xsl:when test="@ind2=2">
                                <span class="label">Hervorgegangen aus:</span>
                            </xsl:when>
                            <xsl:when test="@ind2=3">
                                <span class="label">Teilweise hervorgegangen aus:</span>
                            </xsl:when>
                            <xsl:when test="@ind2=4">
                                <span class="label">Gegründet durch den Zusammenschluss von: ... und: ...</span>
                            </xsl:when>
                            <xsl:when test="@ind2=5">
                                <span class="label">Darin aufgegangen:</span>
                            </xsl:when>
                            <xsl:when test="@ind2=6">
                                <span class="label">Teilweise darin aufgegangen:</span>
                            </xsl:when>
                            <xsl:when test="@ind2=7">
                                <span class="label">Hervorgegangen aus:</span>
                            </xsl:when>
                            <xsl:otherwise>
                                <span class="label">Vorgänger:</span>
                            </xsl:otherwise>
                        </xsl:choose>
                        <xsl:text> </xsl:text>
                    ...
                    <xsl:if test="marc:subfield[@code='n']">
                        <span class="results_summary preceeding_entry_note">
                            <xsl:value-of select="marc:subfield[@code='n']"/>
                        </span>
                    </xsl:if>
            ...
            <xsl:for-each select="marc:datafield[@tag=785]">
                <xsl:if test="@ind1=0">
                    <span class="results_summary succeeding_entry">
                        <xsl:choose>
                            <xsl:when test="@ind2=0">
                                <span class="label">Fortsetzung:</span>
                            </xsl:when>
                            <xsl:when test="@ind2=1">
                                <span class="label">Teilweise Fortsetzung:</span>
                            </xsl:when>
                            <xsl:when test="@ind2=2">
                                <span class="label">Daraus hervorgegangen:</span>
                            </xsl:when>
                            <xsl:when test="@ind2=3">
                                <span class="label">Teilweise daraus hervorgegangen:</span>
                            </xsl:when>
                            <xsl:when test="@ind2=4">
                                <span class="label">Aufgegangen in:</span>
                            </xsl:when>
                            <xsl:when test="@ind2=5">
                                <span class="label">Teilweise aufgegangen in:</span>
                            </xsl:when>
                            <xsl:when test="@ind2=6">
                                <span class="label">Fortsetzung ... und ...:</span>
                            </xsl:when>
                            <xsl:when test="@ind2=7">
                                <span class="label">Zusammengeführt mit ... zu ...</span>
                            </xsl:when>
                            <xsl:when test="@ind2=8">
                                <span class="label">Fortsetzung:</span>
                            </xsl:when>
                            <xsl:otherwise>
                                <span class="label">Nachfolger:</span>
                            </xsl:otherwise>
                        </xsl:choose>
                        <xsl:text> </xsl:text>
        ...
        <xsl:if test="marc:datafield[@tag=787]">
            <span class="results_summary other_relationship_entry">
                <span class="label">Andere related works:</span>
        ...
        <xsl:choose>
            <xsl:when test="$OpacSuppression = 1 and marc:datafield[@tag=942][marc:subfield[@code='n'] = '1']">
                <span class="results_summary suppressed_opac">
                    <span class="label">OPAC-Sicht:</span>
                    Keine Anzeige im OPAC
                </span>
            </xsl:when>
            <xsl:otherwise>
                <xsl:if test="$OPACBaseURL!=''">
                    <span class="results_summary opac_view">
                        <span class="label">OPAC-Sicht:</span>
                        <a>
                            <xsl:attribute name="href"><xsl:value-of select="$OPACBaseURL"/>/cgi-bin/koha/opac-detail.pl?biblionumber=<xsl:value-of
                                    select="str:encode-uri(marc:datafield[@tag=999]/marc:subfield[@code='c'], true())"/>
                            </xsl:attribute>
                            <xsl:attribute name="target">_blank</xsl:attribute>In neuem Fenster öffnen
                        </a>
                        .
                    </span>
                </xsl:if>
                ...
                <xsl:for-each select="$authorfield">
                    <xsl:choose>
                        ...
                        <xsl:when test="@tag=100">
                            <span class="byAuthor">Verfasser*in: </span>
                        </xsl:when>
                        <xsl:when test="@tag=110 or @tag=111">
                            <span class="byAuthor">Herausgeber*in: </span>
                        </xsl:when>
                        <xsl:when test="@tag=245">
                            <span class="byAuthor">Herausgebende Körperschaft: </span>
                        </xsl:when>
                        <!--#13382 Changed Additional author to contributor -->
                        <xsl:when test="@tag&gt;=700">
                            <span class="byAuthor">Mitwirkende(r): </span>
                        </xsl:when>
                    </xsl:choose>
                    <a>
                        <xsl:choose>
                        ...
                        <xsl:choose>
                            ...
                            <xsl:when test="@tag=700 or @tag=710 or @tag=711">
                                ...
                                <xsl:variable name="titleportionfields"
                                              select="boolean(marc:subfield[@code='t' or @code='j' or @code='k' or @code='u'])"/>
    ...
    <xsl:template name="showAuthlink">
        ...
        <xsl:text> </xsl:text>
        <a class='authlink'>
            <xsl:attribute name="href">/cgi-bin/koha/authorities/detail.pl?authid=<xsl:value-of select="$authid"/>
            </xsl:attribute>
            <xsl:attribute name="title">Normsatz anzeigen</xsl:attribute>
            <xsl:attribute name="target">_blank</xsl:attribute>
            <xsl:element name="i">
                <xsl:attribute name="class">fa fa-search</xsl:attribute>
                <xsl:attribute name="aria-label">Normsatz anzeigen</xsl:attribute>
            </xsl:element>
        </a>
    </xsl:template>
    ...
    <xsl:template name="subfieldSelectSubject">
        ...
        <xsl:variable name="str">
            <xsl:for-each select="marc:subfield">
                <xsl:if test="contains($codes, @code)">
                    ...
                    <xsl:value-of select="$prefix"/><xsl:value-of select="translate(text(),'()','')"/><xsl:value-of
        ...
        <xsl:choose>
            <xsl:when test="$urlencode=1">
                <xsl:value-of
                        select="str:encode-uri(substring($str,1,string-length($str)-string-length($delimeter)), true())"/>
            </xsl:when>
            <xsl:otherwise>
